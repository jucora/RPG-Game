...
describe('getLeaderBoard', () => {
    ...
    it('works', async () => {
        const fetchMock = jest
            .spyOn(global, 'fetch')
            .mockImplementation(() => Promise.resolve({ json: () => [{ user: "maria", score: "3000" }, { user: "julian", score: "1200" }] }))

        json = await simulator.getLeaderBoardSimulator()
        expect(fetchMock).toHaveBeenCalledWith(
            'https://us-central1-js-capstone-backend.cloudfunctions.net/api/games/Zl4d7IVkemOTTVg2fUdz/scores/', { "mode": "cors" }
        )
    ...
    describe('getBetterScores', () => {
        it('should take values from API and sort them based on highest score', () => {
            ...
            expect(scoresSorted).toEqual([{ user: "maria", score: "3000" }, { user: "julian", score: "1200" }])
        })
    })
